...
public class Swipe {
    ...
    public void pinchCommandAndroid() {
        Dimension dimension = Drivers.getMobileDriver().manage().window().getSize();

        int height = dimension.height;
        int width = dimension.width;

        double startX = width / 2;
        double upStartY = height * 0.2;
        double downStartY = height * 0.9;

        double finishUpY = height * 0.5;
        double finsihDownY = finishUpY - 10;

        TouchAction actionOne = new TouchAction(Drivers.getMobileDriver());
        actionOne.press(PointOption.point((int) startX, (int) upStartY)).waitAction(WaitOptions.waitOptions(Duration.ofMillis(900))).moveTo(PointOption.point((int) startX, (int) finishUpY)).release();
        TouchAction actionTwo = new TouchAction(Drivers.getMobileDriver());
        actionTwo.press(PointOption.point((int) startX, (int) downStartY)).waitAction(WaitOptions.waitOptions(Duration.ofMillis(900))).moveTo(PointOption.point((int) startX, (int) finsihDownY)).release();
        MultiTouchAction action = new MultiTouchAction(Drivers.getMobileDriver());
    ...
    public void pinchWithStaticPointAndroid() {
        Dimension screenDim = Drivers.getMobileDriver().manage().window().getSize();
        ...
        TouchAction actionOne = new TouchAction(Drivers.getMobileDriver());
        actionOne.press(PointOption.point((int) startX, (int) upStartY)).release();
        TouchAction actionTwo = new TouchAction(Drivers.getMobileDriver());
        actionTwo.press(PointOption.point((int) startX, (int) downStartY)).waitAction(WaitOptions.waitOptions(Duration.ofMillis(900))).moveTo(PointOption.point((int) startX, (int) finsihDownY)).release();
        MultiTouchAction action = new MultiTouchAction(Drivers.getMobileDriver());
        action.add(actionOne);
