...
<#assign scenarioPassCount = 0>
<#assign scenarioCount = scenarios.size()>
...
<#list scenarios as scenario>
	...
	<#assign stepsFailed = 0>
	<#if scenario.getExamplesTable()??> <!-- Examples table scenario steps -->
                <#if scenario.getExamples()??>
                        <#list scenario.getExamples() as example>
                                <#assign steps = scenario.getStepsByExample(example)>
                                <#list steps as step>
                                        <#if step.getFailure()??>
                                                <#assign stepsFailed = stepsFailed + 1>
                                        </#if>
                                </#list>
                        </#list>
                </#if>
		...
		<#if stepsFailed == 0>
			<#assign scenarioPassCount = scenarioPassCount + 1>
		</#if>
...
<div id="header">
	...
	<div id="details">
		...
		<span class="detail">Scenarios Passed:</span> <span class="${scenarioSuccessClass}">${scenarioPassCount} / ${scenarioCount}</span><br/>
		<br/>
		<span class="detail">Environment:</span> ${server!NOT_FOUND}<br/>
		<br/>
		<span class="detail">Build Number:</span> ${buildNumber!NOT_FOUND}<br/>
		<span class="detail">Build Time:</span> ${buildTime!NOT_FOUND}<br/>
		<span class="detail">Build Tag:</span> ${buildTag!NOT_FOUND}<br/>
		<br/>
